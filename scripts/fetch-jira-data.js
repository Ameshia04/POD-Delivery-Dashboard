 ...
 *
 * WHAT'S NEW IN V3:
 *   - Two more Jira issue-link types are now tracked alongside "Blocks":
 *     `dependsOnIssues` ("Dependency" link type -- this issue depends on
 *     that one; sequencing risk, not necessarily a hard stop) and
 *     `relatedIssues` ("Relates" link type -- a loose, informational
 *     association). Each has its own internal/external counts, mirroring
 *     the existing blockedIssues/*BlockerCount pattern. "Fixes", "Issue
 *     split", "Polaris work item link", and "Similar" link types exist on
 *     this Jira site too, but are administrative/discovery-tool links, not
 *     delivery-risk signals, so they're deliberately not tracked here.
 *
...
async function analyzeProject(key, statusCategoryByName) {
  ...
  // Ticket relationships: for each open issue, pull linked issues off three
  // Jira issue-link types -- "Blocks" (a hard blocker), "Dependency" (this
  // issue depends on that one -- sequencing, not necessarily a hard stop),
  // and "Relates" (a loose, informational association). Jira also has
  // "Fixes", "Issue split", "Polaris work item link", and "Similar" link
  // types on this site, but those are administrative/discovery-tool links,
  // not delivery-risk signals, so they're deliberately left out.
  //
  // Every linked issue is tagged with a "scope": if it lives in the same
  // Jira project, it's "internal" (your own team can act on it); if it
  ...
  // needs no extra Jira fields or config. Only links to issues that are
  // still open (not Done) are kept -- a resolved link is no longer a live
  // risk to track.
  function extractLinks(issue, typeName, direction) {
    const links = issue.fields.issuelinks || [];
    const out = [];
    for (const l of links) {
      if (!l.type || l.type.name !== typeName) continue;
      const sides = [];
      if ((direction === "inward" || direction === "both") && l.inwardIssue) sides.push(l.inwardIssue);
      if ((direction === "outward" || direction === "both") && l.outwardIssue) sides.push(l.outwardIssue);
      for (const linked of sides) {
        const bFields = linked.fields || {};
        const bCat = bFields.status && bFields.status.statusCategory ? bFields.status.statusCategory.key : null;
        const isOpen = bCat ? bCat !== "done" : true;
        if (!isOpen) continue;
        const bKey = linked.key;
  ...
  function buildLinkedIssuesList(typeName, direction, fieldName) {
    const list = [];
    for (const issue of wipIssues) {
      const linked = extractLinks(issue, typeName, direction);
      if (linked.length) {
        list.push({ key: issue.key, summary: issue.fields.summary, status: issue.fields.status.name, [fieldName]: linked });
      }
    }
    return list;
  }

  function scopeCounts(list, fieldName) {
    const internal = list.reduce((acc, b) => acc + b[fieldName].filter((x) => x.scope === "internal").length, 0);
    const external = list.reduce((acc, b) => acc + b[fieldName].filter((x) => x.scope === "external").length, 0);
    return { internal, external };
  }

  // Blocks -- inward side ("is blocked by"), i.e. this issue is blocked by the linked one.
  const blockedIssues = buildLinkedIssuesList("Blocks", "inward", "blockedBy");
  ...
  const { internal: internalBlockerCount, external: externalBlockerCount } = scopeCounts(
    blockedIssues.filter((b) => b.blockedBy.length),
    "blockedBy"
  );

  // Dependency -- outward side ("depends on"), i.e. this issue depends on the linked one.
  const dependsOnIssues = buildLinkedIssuesList("Dependency", "outward", "dependsOn");
  const openDependencies = dependsOnIssues.length;
  const { internal: internalDependencyCount, external: externalDependencyCount } = scopeCounts(dependsOnIssues, "dependsOn");

  // Relates -- symmetric link type, so pull both sides ("is related to" / "relates to").
  const relatedIssues = buildLinkedIssuesList("Relates", "both", "relatedTo");
  const openRelated = relatedIssues.length;
  const { internal: internalRelatedCount, external: externalRelatedCount } = scopeCounts(relatedIssues, "relatedTo");
  ...
  return {
    ...
    dependsOnIssues,
    openRelated,
    internalRelatedCount,
    externalRelatedCount,
    relatedIssues,
